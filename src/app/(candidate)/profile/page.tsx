...
function ProfilePageInner() {
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Page header */}
      <div className="sticky top-14 z-40 border-b border-border bg-background/80 backdrop-blur-xl">
        <div className="max-w-5xl mx-auto px-4 h-12 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1 p-1 rounded-xl bg-muted/60 border border-border">
            ...
            <button onClick={() => setMode('edit')}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                mode === 'edit' ? 'bg-indigo-500/20 text-indigo-600 dark:text-indigo-300' : 'text-muted-foreground hover:text-foreground'
              }`}>
              <Pencil size={12} />Edit
            </button>
          </div>
