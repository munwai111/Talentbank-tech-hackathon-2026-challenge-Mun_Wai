...
// Generated by src/lib/ai/persona-profiler.ts from the candidate's resume/CV and
// profile data. Stored inside career_data (JSONB) — indicative, not clinical.

export type PersonaLevel = 'high' | 'moderate' | 'emerging'

export type PersonaAnalysis = {
  persona_summary: string              // whole-person narrative (3-5 sentences)
  tangible_achievements: string[]      // quantified, evidence-backed wins
  professional_engagements: string[]   // roles, communities, leadership involvement
  mbti: {
    type: string                       // e.g. "ENTJ"
    label: string                      // e.g. "The Commander"
    dichotomies: { dimension: string; pole: string; confidence: number; rationale: string }[]
    summary: string
  }
  big_five: { name: string; score: number; insight: string }[]   // OCEAN, score 0-100
  kf4d: {                              // Korn Ferry Four Dimensions-style read
    competencies: { name: string; level: PersonaLevel; insight: string }[]
    drivers:      { name: string; level: PersonaLevel; insight: string }[]
    traits:       { name: string; level: PersonaLevel; insight: string }[]
    experiences: string                // narrative on depth/breadth of experience
  }
