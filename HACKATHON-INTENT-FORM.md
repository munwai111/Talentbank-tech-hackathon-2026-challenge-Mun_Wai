...
*Where the data comes from.* The reference table was constructed by cross-referencing four independently published, annually updated Malaysian salary sources: the **JobStreet Malaysia Salary Report (2024–25)** — the country's largest job portal dataset, drawing from hundreds of thousands of active postings and candidate-submitted figures; the **Robert Walters Malaysia Salary Survey (2025)** — a specialist professional staffing firm with explicit seniority breakdowns for white-collar and tech roles; the **Michael Page Malaysia Salary Guide (2025)** — a second independent recruiter used as a cross-check (where both sources converge, confidence is high; where they diverge, the range is widened to reflect genuine market spread); and the **Department of Statistics Malaysia (DOSM) Labour Force Survey** — the official government wage dataset by occupation, used as a floor-level sanity check to ensure the ranges reflect the actual workforce, not just the premium-employer segment.

*How a range is constructed.* Each band represents approximately the **25th–75th percentile** of Klang Valley-based tech compensation for that role and experience tier. The KL/PJ market is used as the baseline because it concentrates the majority of Malaysian tech hiring. The lower bound of each range is not the minimum — it is the point at which 25% of surveyed professionals in that role and tier earn less. The upper bound is the point at which 25% earn more. A candidate sitting below the lower bound is likely underpaid relative to the market; a candidate above the upper bound is in premium-positioning territory for their experience level.

*How the AI determines what is fair.* The coach does not display the table — it synthesises it against the candidate's specific profile. A "fair" determination runs through five contextual factors applied in sequence: (1) **band placement** — given current salary, role, and years of experience, where does the candidate sit in the reference distribution; (2) **company type adjustment** — MNCs and funded tech companies typically pay 20–40% above local SME rates for equivalent roles; (3) **skills premium** — in-demand capabilities (cloud infrastructure, ML/AI, cybersecurity) carry a 15–30% premium above the base band, which the coach checks against the candidate's Skills Vault; (4) **location adjustment** — Penang and JB roles typically sit 10–20% below the KL benchmark for the same role; (5) **sector weighting** — fintech and e-commerce companies in Malaysia trend above mid-band; government-linked and traditional enterprises typically sit at or below it.

*What "fair" means in practice.* The output is never a single number and never a prediction. It is a contextualised market read: *"Professionals with your skills and experience in this role, in KL, at an MNC, typically earn RM X–Y. You're in the lower third of that band — here is what would move you toward the upper third."* That is an actionable, honest, evidence-grounded signal. It is also the signal most candidates in Malaysia have never received, because the information exists in salary surveys most people never see, and a senior mentor who knows the market is not something most people have access to.

*Singapore context.* For candidates exploring cross-border options: a Malaysian professional at the 75th percentile of their local market typically sits at approximately the 25th–35th percentile of Singapore's equivalent market, at roughly 2.5–3× the MYR figure in SGD. The coach presents this not as a recommendation to leave, but as honest context about the trade-off — including cost-of-living differences — so the candidate can make an informed decision rather than an uninformed one.

*What v1 does vs. what production does.* The current build embeds the curated, research-derived reference table in the coach's system prompt — it does not pull from a live API. This is correct for a prototype: live salary data integration requires a commercial agreement with JobStreet or LinkedIn, which is a production-stage decision. In production, the engine would connect to JobStreet's active posting data (which includes salary ranges on the majority of Malaysian tech postings) for a reference that refreshes as the market does, not once a year when a new survey is published.
